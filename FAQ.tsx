    ...
    <section id="faq" ref={ref} className="py-24 relative overflow-hidden">
      ...
      <div className="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8 pt-16">
        <div className={`text-center mb-14 transition-all duration-700 ${isInView ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-8'}`}>
          ...
          <p className="font-poppins text-base font-medium text-white/70">{t.faq.subtitle}</p>
        </div>
        ...
        <div className="space-y-3">
          {t.faq.items.map((faq, i) => (
            <div
              key={i}
              className={`card rounded-xl overflow-hidden transition-all duration-700 ${isInView ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-4'}`}
              style={{ transitionDelay: `${i * 60}ms` }}
            >
              <button onClick={() => setOpenIndex(openIndex === i ? null : i)} className="w-full flex items-center justify-between p-5 text-left">
                <span className="font-poppins text-base font-bold text-white pr-4">{faq.q}</span>
                <ChevronDown size={18} className={`text-white/30 flex-shrink-0 transition-transform duration-300 ${openIndex === i ? 'rotate-180 text-brand-purple' : ''}`} />
              </button>
              <div className={`overflow-hidden transition-all duration-300 ${openIndex === i ? 'max-h-60 opacity-100' : 'max-h-0 opacity-0'}`}>
                <p className="font-poppins text-sm text-white/70 leading-relaxed px-5 pb-5">{faq.a}</p>
              </div>
            </div>
          ))}
        </div>
